...
GEMINI_API_KEY = 'AIzaSyCn_HyBkc4E9e3SbaMyVgKDTq3sidKNi9k'
# GEMINI_API_KEY = os.environ.get('GEMINI_API_KEY') # Original line
# SERPAPI_API_KEY = os.environ.get('SERPAPI_API_KEY') # Removed
GOOGLE_API_KEY = os.environ.get('GOOGLE_API_KEY')  # Google Search API Key
CUSTOM_SEARCH_ENGINE_ID = os.environ.get(
...
def search_internet(query: str) -> str:
    """
    Performs a Google Custom Search and returns the top 5 results.
    ...
    if not GOOGLE_API_KEY or not CUSTOM_SEARCH_ENGINE_ID:
        return "Error: Google Search API keys are not configured."
    ...
    try:
        ...
        for i, item in enumerate(items):
            ...
            formatted_results.append(
                f"Result {i+1}: Title: {title}. Snippet: {snippet}")
    ...
    try:
        search = GoogleSearch(
            {"q": query, "api_key": SERPAPI_API_KEY, "gl": "us", "hl": "en"})
...
# كمية BNB للشراء (افتراضياً 0.01 BNB)
SNIPE_AMOUNT_BNB = float(os.environ.get('SNIPE_AMOUNT_BNB', 0.01))
...
try:
    ...
    if w3.is_connected() and w3.eth.chain_id == 56:  # 56 هو Chain ID لشبكة BSC
        ...
        try:
            ...
            logging.warning(
                "geth_poa_middleware not found in web3.middleware. Assuming PoA is not needed or handled internally.")
except Exception as e:
...
if w3:
    pancakeswap_router = w3.eth.contract(address=w3.to_checksum_address(
        PANCAKESWAP_ROUTER_ADDRESS), abi=PANCAKESWAP_ABI)
else:
...
# Check every 0.030 seconds for high-speed sniping (as requested by user)
SNIPING_DELAY = 0.030

# Conversation States
ORDER_TYPE, AMOUNT, SYMBOL, PROFIT_PERCENT, USE_STOP_LOSS, STOP_LOSS_PERCENT, LIMIT_PRICE = range(
    7)
GRID_SYMBOL, LOWER_BOUND, UPPER_BOUND, NUM_GRIDS, AMOUNT_PER_ORDER, STOP_GRID_ID = range(
    7, 13)
...
async def early_sniper_task(user_id, chat_id, exchange_id, application):
    ...
    while True:
        try:
                    ...
                    keyboard = [
                        [
                            InlineKeyboardButton(
                                "✅ نعم، اشترِ الآن", callback_data=f"snipe_buy_{symbol}"),
                            InlineKeyboardButton(
                                "❌ لا، تجاهل", callback_data=f"snipe_cancel_{symbol}")
                        ]
                    ]
            ...
            print(
                f"Network error during early sniper for user {user_id} on {exchange_id}. Retrying...")
...
async def find_contract_address(symbol: str) -> str | None:
    ...
    try:
        ...
        if coin_id:
            ...
            contract_address = coin_data.get(
                'platforms', {}).get('binance-smart-chain')
            if contract_address:
                logging.info(
                    f"Found contract address for {symbol} via CoinGecko: {contract_address}")
    ...
    logging.info(
        f"Skipping CoinMarketCap API search for {symbol} (API key not provided).")
    ...
    try:
        ...
        response = requests.get(
            bscscan_search_url, headers=headers, timeout=10)
        response.raise_for_status()
        ...
        contract_link = soup.find(
            'a', href=re.compile(r'/token/0x[a-fA-F0-9]{40}'))

        if contract_link:
            contract_address = contract_link['href'].split('/token/')[1]
            if w3.is_address(contract_address):
                logging.info(
                    f"Found contract address for {symbol} via BscScan Search: {contract_address}")
                return contract_address
...
async def execute_auto_snipe(chat_id: int, symbol: str, contract_address: str, application: Application):
    ...
    try:
        ...
        # A very small number to allow for any amount of tokens (high slippage risk)
        amount_out_min = 1
...
async def handle_sniper_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if action == 'cancel':
        ...
        asyncio.create_task(execute_auto_snipe(
            chat_id, symbol, contract_address, application))
...
async def sniper_early_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    task = asyncio.create_task(early_sniper_task(
        user_id, chat_id, exchange_id, context.application))
...
async def initialize_exchange(exchange_id, api_key, api_secret, password=None):
    ...
    if not exchange_id:
        raise ValueError(
            "Exchange ID is missing. Please use /set_api to select an exchange.")
    ...
    try:
        ...
        raise ValueError(
            f"Unsupported exchange: {exchange_id}. Please select a valid exchange.")
    ...
    params = {
        'apiKey': api_key,
        'secret': api_secret,
        # Assuming spot trading for simplicity
        'options': {'defaultType': 'spot'},
        'enableRateLimit': True,
    }
...
async def execute_trade(update: Update, context: ContextTypes.DEFAULT_TYPE, params):
    ...
    try:
        # CRITICAL FIX: Get password from user record
    ...
    try:
        ...
        if not market:
            raise ccxt.BadSymbol(
                f"Symbol {symbol} market info is not available on {exchange.id}.")

        # Get the current price (best ask for buy order)
        ticker = await exchange.fetch_ticker(symbol)
        current_price = ticker['ask'] if ticker and ticker.get(
            'ask') else ticker['last']
        ...
        amount_to_buy = math.floor(
            amount_to_buy_raw * (10**precision)) / (10**precision)

        if amount_to_buy <= 0:
            raise ccxt.ExchangeError(
                f"Calculated amount to buy ({amount_to_buy}) is zero or less. Check minimum order size.")
        ...
        market_buy_order = await exchange.create_order(
            ...
            type=order_type,
            side='buy',
            # Use calculated amount (or cost for Bitget market)
            amount=amount_for_order,
        ...
        if market_buy_order is None:
            raise ccxt.ExchangeError(
                "Exchange returned a None response for the buy order. Check API keys and permissions.")
        ...
        if not isinstance(market_buy_order, dict) or not market_buy_order.get('id'):
            raise ccxt.ExchangeError(
                "Failed to place market buy order. Order response is incomplete or missing ID.")
        ...
        if order_details is None:
            raise ccxt.ExchangeError(
                f"Failed to fetch order details for ID {order_id}. Order might have been rejected or is missing.")
        ...
        if order_details.get('status') not in ['closed', 'filled']:
            ...
            raise ccxt.ExchangeError(
                f"Buy order failed to fill instantly. Final status: {order_details.get('status') if order_details else 'Unknown'}")
        ...
        if not avg_price or not filled_amount:
            ...
            if not trades:
                raise ccxt.ExchangeError(
                    "Buy order was filled but execution details are missing, and no trades were found.")
            ...
            if not avg_price or not filled_amount:
                raise ccxt.ExchangeError(
                    "Failed to get execution details from order or trades.")
        ...
        if not market:
            raise ccxt.BadSymbol(
                f"Symbol {symbol} market info is not available on {exchange.id}. Cannot set precision.")
        ...
        amount_to_sell = math.floor(
            amount_to_sell_raw * (10**precision)) / (10**precision)

        if amount_to_sell <= 0:
            raise ccxt.InsufficientFunds(
                f"Insufficient available balance ({available_amount} {base_currency}) to place the sell order after fees.")
        ...
        while True:
            # Check every 0.03 seconds for high-speed sniping and monitoring
...
async def handle_profit_withdrawal(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id, amount_usdt_spent, filled_amount, avg_price, target_sell_price, symbol):
    ...
    try:
        ...
        exchange = initialize_exchange(
            user_record['exchange_id'], user_record['api_key'], user_record['api_secret'])
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    welcome_message = get_text(
        user_id, welcome_key, lang=lang, username=username)
    ...
    if user_id in WHITELISTED_USERS:
        if user_id == OWNER_ID:
            welcome_message = get_text(
                user_id, 'welcome_vip_owner', lang=lang, username=username)
            ...
            welcome_message = get_text(
                user_id, 'welcome_vip_other', lang=lang, username=username)
        ...
        keyboard = [
            [InlineKeyboardButton(get_text(
                user_id, 'lang_select_title', lang=lang), callback_data='select_language')]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
    ...
    welcome_message = get_text(
        user_id, 'welcome_client', lang=lang, username=username)
    commands.append(get_text(user_id, 'cmd_status_bot', lang=lang))
    welcome_message += "\n" + "\n".join(commands)
...
async def trade_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    keyboard = [
        [InlineKeyboardButton(get_text(
            user_id, 'trade_market_btn', lang=lang), callback_data='order_type_market')],
        [InlineKeyboardButton(get_text(
            user_id, 'trade_limit_btn', lang=lang), callback_data='order_type_limit')],
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
...
async def set_api_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    for ccxt_id, display_name in EXCHANGES.items():
        ...
        row.append(InlineKeyboardButton(
            display_name, callback_data=f"select_exchange_{ccxt_id}"))
...
async def set_api_secret(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if exchange_id == 'bitget' and 'api_secret' not in context.user_data:
        ...
        # Placeholder to indicate we are now waiting for the secret
        context.user_data['api_secret'] = 'temp_placeholder'
        return WAITING_FOR_API_SECRET
    ...
    try:
        ...
        # CRITICAL FIX: Pass password
        await update_api_keys(user_id, exchange_id, api_key, api_secret, password)
...
async def receive_screenshot(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    keyboard = [[InlineKeyboardButton(
        "✅ تأكيد الاشتراك", callback_data=f'approve_subscription_{user.id}')]]
    reply_markup = InlineKeyboardMarkup(keyboard)
    ...
    try:
        ...
        for i in range(num_grids):
            ...
            if price_precision > 0:
                ...
                buy_price = grid_points[i].quantize(
            ...
            min_amount = Decimal(market['limits']['amount']['min']
                                 ) if market['limits']['amount']['min'] is not None else Decimal('0')
            ...
            min_notional = Decimal(
                market['limits']['cost']['min']) if market['limits']['cost']['min'] is not None else Decimal('0')
...
async def get_grid_id_to_stop(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    user_grids = await get_user_grids(user_id)
    target_grid = next(
        (g for g in user_grids if g['id'] == grid_id and g['status'] == 'active'), None)
...
async def get_use_stop_loss(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if response in ['yes', 'نعم', 'y', 'ن']:
        ...
        if context.user_data.get('is_sniping'):
            asyncio.create_task(sniping_and_trade(
                update, context, context.user_data))
        else:
...
async def grid_monitoring_task(application: Application, grid_id: int):
    ...
    while True:
        try:
            ...
            buy_order_open = any(
                order['side'] == 'buy' and
                Decimal(str(order['price'])).quantize(
                    quantizer_price) == next_buy_price.quantize(quantizer_price)
                for order in open_orders
            )
            ...
            if next_buy_price and not buy_order_open:
                ...
                if sell_price <= upper_bound:
                    ...
                    sell_amount_base = sell_amount_base.quantize(
                        quantizer_amount, rounding=ROUND_HALF_UP)

                    # Convert Decimal back to float for ccxt
                    sell_price_float = float(sell_price.quantize(
                        quantizer_price, rounding=ROUND_HALF_UP))
                ...
                if new_buy_price >= lower_bound:
                    ...
                    # Apply precision rounding
                    quantizer_amount = Decimal(f'1e-{amount_precision}')
                    buy_amount_base = buy_amount_base.quantize(
                        quantizer_amount, rounding=ROUND_HALF_UP)

                    # Convert Decimal back to float for ccxt
                    new_buy_price_float = float(new_buy_price.quantize(
                        quantizer_price, rounding=ROUND_HALF_UP))
                    buy_amount_float = float(buy_amount_base)
            ...
            sell_order_open = any(
                order['side'] == 'sell' and
                Decimal(str(order['price'])).quantize(
                    quantizer_price) == next_sell_price.quantize(quantizer_price)
                for order in open_orders
            )
            ...
            if next_sell_price and not sell_order_open:
                ...
                if buy_price >= lower_bound:
                    ...
                    # Convert Decimal back to float for ccxt
                    buy_price_float = float(buy_price.quantize(
                        quantizer_price, rounding=ROUND_HALF_UP))
                    buy_amount_float = float(buy_amount_base)
                ...
                if new_sell_price <= upper_bound:
                    ...
                    # Convert Decimal back to float for ccxt
                    new_sell_price_float = float(new_sell_price.quantize(
                        quantizer_price, rounding=ROUND_HALF_UP))
                    sell_amount_float = float(sell_amount_base)
            ...
            try:
                ...
                print(
                    f"Failed to send error message to user {user_id}: {msg_e}")
        finally:
            if 'exchange' in locals():
                await exchange.close()
...
async def global_grid_manager_loop(application: Application):
    ...
    while True:
        try:
            ...
            tasks_to_stop = [
                grid_id for grid_id in active_grid_tasks if grid_id not in active_grid_ids]
...
def main() -> None:
    ...
    subscription_conv_handler = ConversationHandler(
        entry_points=[CallbackQueryHandler(
            subscribe_callback, pattern='^subscribe_now$')],
        states={
    ...
    trade_conv_handler = ConversationHandler(
        entry_points=[CommandHandler("trade", trade_start), CommandHandler(
            "sniping", sniping_start)],
        states={
    ...
    application.add_handler(CommandHandler(
        "sniper_early", sniper_early_command))
    application.add_handler(CommandHandler(
        "stop_sniper_early", stop_sniper_early_command))
    application.add_handler(CommandHandler("cancel", simple_cancel_command))
    application.add_handler(CallbackQueryHandler(
        approve_subscription_callback, pattern='^approve_subscription_'))
    ...
    # Language Selection Handlers
    application.add_handler(CallbackQueryHandler(
        language_callback_handler, pattern='^select_language$'))
    application.add_handler(CallbackQueryHandler(
        set_language, pattern='^set_lang_'))

    # Add handler for early sniper callbacks
    application.add_handler(CallbackQueryHandler(
        handle_sniper_callback, pattern='^snipe_(buy|cancel)_'))
    ...
    def run_web_server():
        ...
        try:
            ...
            print(
                f"Waitress not found. Starting Flask development server on port {PORT}...")
    ...
    application.run_polling(
        poll_interval=1.0, allowed_updates=Update.ALL_TYPES)
